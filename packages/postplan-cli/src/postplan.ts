...
const CONFIG_KEYS = ["project", "endpoint", "token"] as const;
...
function writeJson(path: string, value: Config): void {
...
function parsePlanFilename(path: string): ParsedPlanFilename {
...
function injectHostedLink(html: string, { hostedUrl, localPath }: HostedLinkInput): string {
...
function createPlan(args: string[]): void {
...
function setConfig([key, ...valueParts]: string[]): void {
...
function getPublishedUrl(body: unknown): string {
...
async function publish(args: string[]): Promise<void> {
  ...
  async function sendPublish(currentHtml: string): Promise<string> {
